class ProductLoader {
  constructor({ jsonPath, containerId, productsPerPage = 20 }) {
    ...
    this.prevButton = this.prevButton = this.createPaginationButton(
      'Previous',
  ...
  async init() {
    try {
      ...
      this.allProducts = data.filter(
        (p) =>
          p.categorie.toLowerCase() === 'packaging - box' ||
          p.categorie.toLowerCase() === 'packaging - bag'
      );
      this.updatePagination(); // Initial load of packaging items
    } catch (error) {
